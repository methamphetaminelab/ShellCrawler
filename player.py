import curses
from dungeonGenerator import generateChests, generateEnemies
import random
...
def movePlayer(dungeon, key, roomsCoords):
    ...
    if dungeon[x][y] in ['-', '|']:
        dungeon = markClosestRoom(dungeon, playerPos, roomsCoords)

    dungeon[x][y] = "+"
    return dungeon

def markClosestRoom(dungeon, playerPos, roomsCoords):
